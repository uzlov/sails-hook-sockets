...
module.exports = function ToReceiveIncomingSailsIOMsg(app) {
  ...
  return function receiveIncomingSailsIOMsg(options) {
    ...
    function respondWithParseError (detailedErrorMsg) {
      ...
      if (['debug', 'warn', 'error'].indexOf(app.config.log.level) > -1) {
        // Reset all properties, except "status" and "message".
        for (var key in error) if (['status', 'message'].indexOf(key) === -1) delete error[key];
      }
      // Log parse error
      app.log.error(error);
